...
RegisterNetEvent('dag_goldpanning:client:StartPaning')
AddEventHandler('dag_goldpanning:client:StartPaning', function()
    if not Panning then 
        Panning = true
            ...
            if Water == WaterTypes2[k]["waterhash"]  then
				if IsPedOnFoot(PlayerPedId()) then
                    if IsEntityInWater(PlayerPedId()) then
						...
						ClearPedTasks(ped)
						GoldShake()
						exports['qbr-core']:Notify(9, Config.oro_1, 15000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
						Wait(15000)
						ClearPedTasks(ped)
						DeleteObject(entity2)
						DeleteEntity(entity2)
						TriggerServerEvent("dag_goldpanning:server:search")
						break
					end
				end
            end
        ...
        if foundwater == false then
            exports['qbr-core']:Notify(9, Config.oro_no_recoger, 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
    end
end)
